 ...
 *
 *  $Id: callback.c 144 2012-11-14 18:09:43Z krentel $
 */
...
/*Edit start by Probir for malloc and free*/
void __attribute__ ((weak))
...
{
    MONITOR_DEBUG("(default callback) size = %d, handle = %p\n",
		  size, handle);
}
...
void __attribute__ ((weak))
monitor_pre_valloc(size_t size)
{
    MONITOR_DEBUG1("(default callback)\n");
}
void __attribute__ ((weak))
monitor_post_valloc(size_t size, void *ptr)
{
    MONITOR_DEBUG1("(default callback)\n");
}
void __attribute__ ((weak))
...
void __attribute__ ((weak))
monitor_post_posix_memalign(void** memptr, size_t alignment, size_t size, int ret)
{
    MONITOR_DEBUG1("(default callback)\n");
}
