...
//	printf("running %d \n",ThreadEventMap[0].total);
    ...
    int a = 0;
    int b = 0;
    if(SamplingThreshold == 171)
    {
	a = 131; b = 211;
//	a = 97;  b = 170;
    }
    else if (SamplingThreshold == 264)
    {
        a = 211; b = 317;
    }
    else if (SamplingThreshold == 494)
    {
        a = 419; b = 569;
    }
    else if (SamplingThreshold == 705)
    {
        a = 653; b = 757;
    }else if (SamplingThreshold == 1212)
    {
        a = 997; b = 1427;
    }
    else if (SamplingThreshold == 4812)
    {
        a = 4793; b = 4831;
    }
    else if(SamplingThreshold == 10151)
    {
        a = 10051; b = 10251;
    }
    else
    ...
    int r1 = Equilikely(a, b);
       /* read the counter values and store them in the values array */
//   if ( (retval=PAPI_read(EventSet, values)) != PAPI_OK)
  //    ERROR_RETURN(retval);
    if ( (retval=PAPI_stop (thisEventSet_p, values)) != PAPI_OK)
      ERROR_RETURN(retval);    
    //printf("FROM PEBS SMAPLING HANDLER=========================> PROCESS # of sampled events %ld\n",values[0]);
    {
	if ((retval = PAPI_overflow (thisEventSet_p, ThreadEventMap[EventSet].PAPI_event, r1, 0, handlerForProcess)) != PAPI_OK)
              ERROR_RETURN(retval);
    }
    //printf("FROM PEBS SMAPLING HANDLER=========================> PROCESS Setting threshold %d range %d - %d\n",r1,a,b);
...
//	printf("FROM PEBS SMAPLING HANDLER=========================> THREAD address %x\n", address);
	...
	printf("FROM PEBS SMAPLING HANDLER=========================> THREAD # of sampled events %ld\n",values[0]);
#endif
	//ThreadEventMap[MapIndex].unWindZone = 3;
}
