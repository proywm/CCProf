      ...
      if ( native ) {
			...
			if ((aName == NULL)  ||  (strstr(aName, ":k=") == NULL)) {
				...
				pe_ctl->events[i].attr.exclude_kernel = !(pe_ctl->domain & PAPI_DOM_KERNEL);
      			...
      			// Xu Liu: give the accurate IP
      			pe_ctl->events[i].attr.precise_ip=2;
			
			if (strstr(aName,"LATENCY_ABOVE_THRESHOLD") != NULL)
  			{
    				pe_ctl->events[i].attr.type = 0x4;
    				pe_ctl->events[i].attr.config1 = 3;
...
_pe_dispatch_timer( int n, hwd_siginfo_t *info, void *uc)
{
  ...
  if ( thread->running_eventset[cidx]->overflow.flags & 
    ...
    _papi_hwi_dispatch_overflow_signal( ( void * ) &hw_context, 
					address, (caddr_t)0, 0, 0, 0, 0, 0, NULL, 0,
					0, &thread, cidx );
    return;
  }
...
//    ip = *( uint64_t * ) ( data + ( ( head - 8 ) & pe->mask ) );
      ...
      uint64_t weight;
      uint64_t src;
      addr = *( uint64_t * ) ( data + ( ( head - 32 ) & pe->mask ) );
      cpu = *( uint64_t * ) ( data + ( ( head - 24 ) & pe->mask ) );
      time = *( uint64_t * ) ( data + ( ( head - 40 ) & pe->mask ) );
      tid = *( uint64_t * ) ( data + ( ( head - 48 ) & pe->mask ) );
      ip = *( uint64_t * ) ( data + ( ( head - 56 ) & pe->mask ) );
      weight = *( uint64_t * ) ( data + ( ( head - 16 ) & pe->mask ) );
      src = *( uint64_t * ) ( data + ( ( head - 8 ) & pe->mask ) );
...
_pe_set_overflow( EventSetInfo_t *ESI, int EventIndex, int threshold )
{
  ...
  case WAKEUP_MODE_COUNTER_OVERFLOW:
    ...
    ctl->events[evt_idx].attr.sample_type = PERF_SAMPLE_IP | PERF_SAMPLE_ADDR | PERF_SAMPLE_CPU | PERF_SAMPLE_TID | PERF_SAMPLE_TIME | PERF_SAMPLE_WEIGHT | PERF_SAMPLE_DATA_SRC;
