...
#define PAD1 1
#define PAD2 0
#define MR(mt,n,r,c,d)  mt->m[(n) * (mt->mrows+PAD2) * (mt->mcols+PAD1) * (mt->mdeps+PAD) + (r) * (mt->mcols+PAD1) * (mt->mdeps+PAD) + (c) * (mt->mdeps+PAD) + (d)]
...
newMat(Matrix* Mat, int mnums,int mrows, int mcols, int mdeps)
{
  ...
  Mat->m= NULL;
  Mat->m= (float*) 
    malloc(mnums * (mrows+PAD2) * (mcols+PAD1) * (mdeps+PAD) * sizeof(float));
