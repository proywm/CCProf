...
static void init_r(double *x, int N1, int N2, int N3, int H1, int H2, int H3);
static void init_c(MKL_Complex16 *x, int N1, int N2, int N3, int H1, int H2,
                   int H3);
static int verify_r(double *x, int N1, int N2, int N3, int H1, int H2, int H3);
static int verify_c(MKL_Complex16 *x, int N1, int N2, int N3, int H1, int H2,
                    int H3);
...
int main(void)
{
    ...
    x = (double *)mkl_malloc(N1*N2*(N3/2+1)*2*sizeof(double), 64);
    if (0 == x) goto failed;
    ...
    status = verify_c((MKL_Complex16*)x, N1, N2, N3, H1, H2, H3);
    if (0 != status) goto failed;
    ...
    init_c((MKL_Complex16*)x, N1, N2, N3, H1, H2, H3);
...
static void init_r(double *x, int N1, int N2, int N3, int H1, int H2, int H3)
{
    ...
    factor = (2*(N1-H1)%N1==0 && 2*(N2-H2)%N2==0 && 2*(N3-H3)%N3==0) ? 1.0 : 2.0;
    for (n1 = 0; n1 < N1; n1++)
    {
        for (n2 = 0; n2 < N2; n2++)
        {
...
static int verify_c(MKL_Complex16 *x,int N1,int N2,int N3,int H1,int H2,int H3)
{
     ...
     * input data
     */
    errthr = 2.5 * log( (double)N1*N2*N3 ) / log(2.0) * DBL_EPSILON;
...
static void init_c(MKL_Complex16 *x,int N1,int N2,int N3,int H1,int H2,int H3)
{
