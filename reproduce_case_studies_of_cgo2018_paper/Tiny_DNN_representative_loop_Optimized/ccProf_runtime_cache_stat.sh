...
BENCHMARK_BINARY=$BENCHMARK_RELATIVE_LOCATION/loop_pad_O0
BENCHMARK_BINARY_O3=$BENCHMARK_RELATIVE_LOCATION/loop_pad_O3
mkdir workspace
cd workspace
...
starttime=$(date +%s.%N)
OMP_NUM_THREADS=1 $BENCHMARK_BINARY
endtime=$(date +%s.%N)
...
starttime=$(date +%s.%N)
OMP_NUM_THREADS=1 $BENCHMARK_BINARY_O3
endtime=$(date +%s.%N)
...
OMP_NUM_THREADS=1 $CCPROFDIR/PAPI/PAPI/src/libpfm4/perf_examples/task -e L1-DCACHE-LOADS -e L1-DCACHE-LOAD-MISSES -e LLC_MISSES -e LLC_REFERENCES $BENCHMARK_BINARY_O3 > cacheOutScratch
