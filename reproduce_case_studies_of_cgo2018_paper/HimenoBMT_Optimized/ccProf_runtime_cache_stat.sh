...
starttime=$(date +%s.%N)
OMP_NUM_THREADS=1 $BENCHMARK_BINARY  S 20
endtime=$(date +%s.%N)
...
starttime=$(date +%s.%N)
OMP_NUM_THREADS=1 $BENCHMARK_BINARY_O3  S 20
endtime=$(date +%s.%N)
...
OMP_NUM_THREADS=1 $CCPROFDIR/PAPI/PAPI/src/libpfm4/perf_examples/task -e L1-DCACHE-LOADS -e L1-DCACHE-LOAD-MISSES -e LLC_MISSES -e LLC_REFERENCES $BENCHMARK_BINARY_O3  S 20 > cacheOutScratch
